       ...
       Sys.Application.add_load(function () {
        ...
        function RadToolBar1_ButtonClicking(sender, args) {
            ...
            if (value == "new") {
                ...
                var msgClear ='';
                var deleteCheckMsg = '<%= lblDeleteCheckMsg.Text %>';
                var deleteFailMsg = '<%= lblDeleteFailMsg.Text %>';
                if(status =="receive"){
                    msgClear = $uof.tool.htmlDecode('<%=System.Web.Security.AntiXss.AntiXssEncoder.HtmlEncode(lblClearReceiveboxMsg.Text,true)%>');
                }
                else if(status =="send"){
                    msgClear = $uof.tool.htmlDecode('<%=System.Web.Security.AntiXss.AntiXssEncoder.HtmlEncode(lblClearSendboxMsg.Text,true)%>');
                }

                var userInput = prompt(msgClear);
                if (userInput === null) {
                    // 用戶按了取消，不執行任何操作
                    args.set_cancel(true);
                } else if (userInput !== deleteCheckMsg) {
                    alert(deleteFailMsg);
                    args.set_cancel(true);
                }
            }
                    ...
                    <ContentTemplate>
                        <telerik:RadToolBar ID="RadToolBar1" Width="100%" runat="server" OnClientButtonClicking="RadToolBar1_ButtonClicking" OnButtonClick="RadToolBar1_ButtonClick" meta:resourcekey="RadToolBar1Resource1" SingleClick="None">
                            <Items>
                                ...
                                <telerik:RadToolBarButton runat="server" Text="" Value="clearAll" Enabled="False"
                                    ClickedImageUrl="~/Common/Images/Icon/icon_m03.gif" DisabledImageUrl="~/Common/Images/Icon/icon_m03.gif"
...
<asp:Content ID="Content4" ContentPlaceHolderID="RightContentPlaceHolder" runat="Server">
    ...
    <asp:Label ID="lblDeleteCheckMsg" runat="server" Text="我同意" Visible="False" meta:resourcekey="lblDeleteCheckMsgResource1"></asp:Label>
    <asp:Label ID="lblDeleteFailMsg" runat="server" Text="輸入內容不正確，請重新操作！" Visible="False" meta:resourcekey="lblDeleteFailMsgResource1"></asp:Label>
    <asp:Label ID="lblClearReceiveboxMsg" runat="server" Text="將清空收件匣中所有的私人訊息，包含已歸檔的資料，且清空後無法復原，確定要清空？請再次輸入「我同意」，才可清空收件匣" Visible="False" meta:resourcekey="lblClearReceiveboxMsgResource1"></asp:Label>
    <asp:Label ID="lblClearSendboxMsg" runat="server" Text="將清空寄件匣中所有的私人訊息，包含已歸檔的資料，且清空後無法復原，確定要清空？請再次輸入「我同意」，才可清空寄件匣" Visible="False" meta:resourcekey="lblClearSendboxMsgResource1"></asp:Label>
