...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    ...
    var labHelp = $("#<%=labHelp.ClientID%>");
            ...
            if (fileID != null && fileID != "" && fileID != "WorksheetNameError") {
                labHelp.html("<font color='blue'>" + "<%=System.Web.Security.AntiXss.AntiXssEncoder.HtmlEncode(lblSuccess.Text, true) %>" + "</font>");
            }
            else if (fileID == "WorksheetNameError") {
                var lblError = "<%= System.Web.Security.AntiXss.AntiXssEncoder.HtmlEncode(lblError.Text, true) %>";
                var lblWorksheetNameError = "<%= System.Web.Security.AntiXss.AntiXssEncoder.HtmlEncode(lblWorksheetNameError.Text, true) %>";

                labHelp.html("<font color='red'>"
                    + lblError
                    + '<br>'
                    + lblWorksheetNameError
                    + "</font>");
            }
            else
        ...
        function downloadFile() {
            ...
            if (pd) {
                $("body", pd).after("<iframe style=display:none src=Common/DownloadFile.aspx?dlFileName=MyMeeting.xlsx&id=" + fileID + "></iframe>")
            }
    ...
    <asp:Label ID="lblError" runat="server" Text="匯出失敗!" Visible="False" meta:resourcekey="lblErrorResource1"></asp:Label>
    <asp:Label ID="lblWorksheetNameError" runat="server" Text="所選的會議主旨含有Excel工作表名稱不支援的字元： \ / ? * : [ ]" Visible="False" meta:resourcekey="lblWorksheetNameErrorResource1"></asp:Label>
